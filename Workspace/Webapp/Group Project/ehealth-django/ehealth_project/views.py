...
def user_profile(request,username,current_folder=None):
    ...
    if request.method == 'POST':
        ...
        if(request.POST.get('del-btn')):
            id = request.POST.get('del-page')
    ...
    users_folders = Folder.objects.all().filter(user=user_prof, privacy=False)
    ...
    context_dict={'user_prof':user_prof,'users_public_folders':users_folders,'current_pages':current_pages, 'current_folder':current_folder, 'current_users_profile': current_users_profile}
